...
import TopBar from '../common/nav/TopBar.js';
...
describe('components/TopBar - logged in', () => {
  beforeEach(() => {
    ...
    const wrapper = mount(
      <ApolloProvider client={client}>
        <MockedProvider mocks={mocks} addTypename={false}>
          <Router history={history}>
            <TopBar id="google-oauth2|2147627834623744883746" />
          </Router>
        </MockedProvider>
      </ApolloProvider>
    );
  ...
  it('Renders with correct userId', async () => {
    ...
    expect(wrapper).toBeTruthy();
    expect(
      wrapper.contains(<p className="top-bar-user-info">buckbulldog</p>)
    ).toEqual(false);
  });
});
