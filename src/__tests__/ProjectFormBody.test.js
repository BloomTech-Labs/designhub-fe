...
import ProjectFormBody from '../views/CreateProject/ProjectFormBody.js';
...
describe('components/ProjectFormBody - logged in', () => {
  beforeEach(() => {
    ...
    const wrapper = mount(
      <ApolloProvider client={client}>
        <MockedProvider mocks={mocks} addTypename={false}>
          <Router history={history}>
            <ProjectFormBody id="google-oauth2|2147627834623744883746" />
          </Router>
        </MockedProvider>
      </ApolloProvider>
    );
...
it('Renders with correct components', async () => {
  ...
  expect(wrapper).toBeTruthy();
  // should contain a button to be defined
  expect(wrapper.find('DeleteProjectModal')).toHaveLength(1);
  expect(wrapper.find('InviteModal')).toHaveLength(1);
  expect(wrapper.find('MultiImageUpload')).toHaveLength(1);
